...
#cross_merge1= read.csv('./4_gather.thresholds/FTICR_crosstable_rep.merged1_all_em.thres_2022-05-05.csv')
names(cross_merge1)
...
cross_merge2$Class=recode(cross_merge2$Class, "ConHC" = "Condensed aromatics", "UnsatHC" = "Unsaturated hydrocarbons", "Carb" = "Carbohydrates")
...
cross_merge2$Class<- factor(cross_merge2$Class, levels = c("AminoSugar", "Carbohydrates", "Condensed aromatics", "Lignin", "Lipid", "Protein", "Tannin", "Unsaturated hydrocarbons", "Other"))
...
g_water=ggplot(cross_merge2, aes(x = cs.flag.emergent_water, fill = Class)) + theme_classic()+
...
ggsave("Fig.1_MFclass_histogram_FINAL_11-09.png", dpi=300, height = 5, width=8, g)
#ggsave("./Fig1_general.CS_VK_barplot/Fig.1_MFclass_histogram_FINAL_11-09.png", dpi=300, height = 5, width=8, g)
...
##Water
ggplot(cross_merge2, aes(x = cs.flag.emergent_water, fill = Class)) +theme_classic()+
  geom_bar(position = "fill") +labs(title="Water", X=" ", y="Relative Contribution (%)")+
  theme(aspect.ratio=1, strip.text.x = element_text(size = 12), axis.title.y = element_text(size = 14), axis.text.y = element_text(size = 12), axis.title.x = element_blank(), axis.text.x = element_text(size = 12))+
  scale_fill_manual(values = c("#88CCEE", "#CC6677", "#DDCC77","#44AA99", "#117733", "#332288", "#AA4499", "#999933", "#661100"))+
  theme(legend.position = "none")+
